...
export const saveChunkToDB = async (chunk: AudioChunk, duration: number) => {
  ...
  // We slaan de duur op als een schoon getal. Als het NaN is, gebruiken we 0.
  const validDuration = isNaN(duration) ? 0 : Math.floor(duration);
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      const results = (request.result as SessionMetadata[])
        .filter(s => s.lastUpdated > fortyEightHoursAgo && s.duration > 0);
      resolve(results);
    };
    request.onerror = () => reject(request.error);
  });
