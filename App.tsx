...
const App: React.FC = () => {
  ...
  /**
   * CRITICAL RE-ASSESSMENT: Calculating accurate duration for recovery.
   * Browsers often fail to provide metadata for WebM blobs generated by MediaRecorder.
   * Using AudioContext to decode the actual audio data is the most robust way to get the true length.
   */
  const getAccurateAudioDuration = async (blob: Blob): Promise<number> => {
    try {
      ...
      console.error("Accurate duration decoding failed:", e);
      return 0;
    }
  };
  ...
  const handleRecover = async () => {
    ...
    try {
        setIsGoogleBusy(true); // Freeze UI and show loading
        ...
        if (chunks && chunks.length > 0) {
          ...
          // Use the most accurate method to determine duration from file data
          addLog("Calculating file duration...");
          ...
          // Prefer accurate decoded duration, fallback to metadata
